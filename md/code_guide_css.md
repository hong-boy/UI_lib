...
```
### 媒体查询（Media query）的位置|css_media_queries
...
```
### 带前缀的属性|css_prefixed_properties
当使用特定厂商的带有前缀的属性时，通过缩进的方式，让每个属性的值在垂直方向对齐，这样便于多行编辑。

在 Textmate 中，使用 **Text &rarr; Edit Each Line in Selection** (&#8963;&#8984;A)。在 Sublime Text 2 中，使用 **Selection &rarr; Add Previous Line** (&#8963;&#8679;&uarr;) 和 **Selection &rarr; Add Next Line** (&#8963;&#8679;&darr;)。
...
```
### 单行规则声明|css_single_declarations
...
大部分情况下，我们不需要为简写形式的属性声明指定所有值。例如，HTML 的 heading元素只需要设置上、下边距（margin）的值，因此，在必要的时候，只需覆盖这两个值就可以。过度使用简写形式的属性声明会导致代码混乱，并且会对属性值带来不必要的覆盖从而引起意外的副作用。
